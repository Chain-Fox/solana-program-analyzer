                ...
                if let Some(mut idom) = new_idom {
                    ...
                    for pred in predecessors {
                        if pred != &idom && immediate_dominators.contains_key(pred) {
                            idom = Self::intersect(&immediate_dominators, &reverse_postorder, idom, pred.clone());
                        }
                    }
...
mod tests2 {
    ...
    fn test_postdom_complex_graph() {
        ...
        // Check immediate post-dominators
        assert_eq!(postdominators.immediate_post_dominator(&"A"), ExtNode::Real(Some("F")));
        assert_eq!(postdominators.immediate_post_dominator(&"B"), ExtNode::Real(Some("F")));
        assert_eq!(postdominators.immediate_post_dominator(&"C"), ExtNode::Real(Some("F")));
        assert_eq!(postdominators.immediate_post_dominator(&"D"), ExtNode::Real(Some("F")));
        assert_eq!(postdominators.immediate_post_dominator(&"E"), ExtNode::Real(Some("F")));
        assert_eq!(postdominators.immediate_post_dominator(&"F"), ExtNode::Real(Some("F")));
    }
    ...
    fn test_nearest_common_post_dominator() {
        ...
        assert_eq!(postdominators.nearest_common_post_dominator(&"B", &"C"), Some("G"));
        assert_eq!(postdominators.nearest_common_post_dominator(&"D", &"E"), Some("G"));
        assert_eq!(postdominators.nearest_common_post_dominator(&"E", &"F"), Some("G"));
        assert_eq!(postdominators.nearest_common_post_dominator(&"A", &"G"), Some("G"));
        
        // Same node should return itself
        assert_eq!(postdominators.nearest_common_post_dominator(&"A", &"A"), Some("A"));
        assert_eq!(postdominators.nearest_common_post_dominator(&"G", &"G"), Some("G"));
    }
}